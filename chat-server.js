	...
	UsersModel.findOneAndUpdate({ userId: userId }, dataToUpdate, {
		...
		if (updated_user) {
			...
			Object.keys(allConnections).forEach((element) => {
				allConnections[element].sendUTF(responseSuccess(200, "userModified", updated_user, "Online/Offline Status Changed", true));
			});
		}
	});
}
...
function createNewRoomNotify(savedMessage) {
	...
	UsersModel.find(fondData, (err, userList) => {
		...
		savedMessage.userList.forEach(element => {
			...
			if (allConnections[element]) {
				allConnections[element].sendUTF(responseSuccess(200, "createRoom", responseData, "New Room Created", true));
			}
			...
			UsersModel.findOneAndUpdate({ /* _id: requestData._id,  */
				...
				UsersModel.find({ _id: mongoose.Types.ObjectId(updated_user._id) }, (err, findUser) => {
					...
					if (findUser && findUser.length > 0) {
						...
						Object.keys(allConnections).forEach((element) => {
							allConnections[element].sendUTF(responseSuccess(200, "userModified", findUser[0], "User Details Changed", true));
						});
					}
				});
			...
			userList: Array */
						RoomModel.findOneAndUpdate({ _id: mongoose.Types.ObjectId(room._id) }, newMessageInfo, {
							...
							if (err) {
								...
								room.userList.forEach(user => {
									if (allConnections.hasOwnProperty(user)) {
										allConnections[user].sendUTF(responseSuccess(200, "roomsModified", updatedRoom, "Modified", true));
									} else {
						...
						room.userList.forEach(user => {
							if (allConnections.hasOwnProperty(user)) {
								...
								allConnections[user].sendUTF(responseSuccess(201, "message", formattedMessages, "Data Found", true));
	  ...
	  "message": "asdasdasd"
			...
			if (validation.fails()) {
				...
				MessageModel.findOneAndUpdate({ _id: mongoose.Types.ObjectId(requestData.messageId) }, dataToUpdate, {
					...
					if (data) {
						Object.values(allConnections).forEach((connectionElement) => {
							connectionElement.sendUTF(responseSuccess(200, "updateMessage", data, "Modified", true));
						});
						// connection.sendUTF(responseSuccess(200, "updateMessage", data, "Modified", true));
					}
				});
			}
		}
				...
				if (data) {
					...
					BlockModel.find({
						...
						if (data) {
							console.log("allBlockUser", data);
							// Notify to all active user about that user status
							allConnections[requestData.blockedBy] && allConnections[requestData.blockedBy].sendUTF(responseSuccess(200, "blockUser", data[0], "Block Status Changed", true));
							allConnections[requestData.blockedTo] && allConnections[requestData.blockedTo].sendUTF(responseSuccess(200, "blockUser", data[0], "Block Status Changed", true));
